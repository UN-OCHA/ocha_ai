...
/**
 * AWS Bedrock Nova 2 v1 completion generator base class.
 */
abstract class AwsBedrockNova2V1Base extends AwsBedrock {
   ...
   * @var string[]
   */
  protected const THINKING_MODES = ['none', 'low', 'medium', 'high'];
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state): array {
    ...
    $form['plugins'][$plugin_type][$plugin_id]['thinking_mode'] = [
      '#type' => 'select',
      '#title' => $this->t('Extended thinking mode'),
      '#description' => $this->t('Nova 2 extended thinking controls how much the model reasons before answering. "None" uses efficient latent reasoning; low/medium/high enable explicit step-by-step reasoning for increasingly complex tasks. When set to "high", temperature and top_p are not sent (per API requirements).'),
   ...
   * {@inheritdoc}
   *
   * Supported keys in $parameters (override plugin settings):
   * - max_tokens: (int) Maximum tokens to generate.
   * - temperature: (float) Sampling temperature.
   * - top_p: (float) Top-p sampling.
   * - thinking_mode: (string) Extended thinking level: none, low, medium, or
   *   high.
   */
  protected function generateRequestBody(string $prompt, string $system_prompt = '', array $files = [], array $parameters = []): array {
    $thinking_mode = $parameters['thinking_mode'] ?? $this->getPluginSetting('thinking_mode', 'none');
    $thinking_mode = in_array($thinking_mode, self::THINKING_MODES, TRUE) ? $thinking_mode : 'none';

    // When thinking mode is "high", AWS requires temperature, topP and
    // maxTokens to be unset.
    $is_high_effort = ($thinking_mode === 'high');
    ...
    if ($thinking_mode !== 'none') {
      $payload['additionalModelRequestFields'] = [
        'reasoningConfig' => [
          'type' => 'enabled',
          'maxReasoningEffort' => $thinking_mode,
        ],
      ];
    }
